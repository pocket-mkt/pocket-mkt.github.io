...
function DailyMeetingsView({ role, meetings, canWrite, onSave }) {
  ...
  return <div className="view-stack daily-meeting-view"><ViewHeader eyebrow="업무 기록" title="데일리 회의록" description="날짜별 논의 내용, 결정사항과 후속 업무를 한곳에 남깁니다.">{canWrite && <button className="primary-button" type="button" onClick={() => setEditing(null)}><Plus size={15} /> 회의록 작성</button>}</ViewHeader>
    ...
    {meetings.length ? <section className="daily-meeting-list">{meetings.map((meeting) => <article className="daily-meeting-card panel" key={meeting.id}><header><div className="daily-meeting-date"><CalendarDays size={17} /><span>{meeting.date}</span></div><div><span className="daily-meeting-visibility">{visibilityLabel[meeting.visibilityCode] || meeting.visibilityCode}</span>{canWrite && <button className="icon-button" type="button" onClick={() => setEditing(meeting)} aria-label={`${meeting.title} 수정`}><Pencil size={15} /></button>}</div></header><div className="daily-meeting-title"><h3>{meeting.title}</h3><span>{meeting.authorName}{meeting.attendees ? ` · 참석 ${meeting.attendees}` : ""}</span></div><div className="daily-meeting-sections"><section><h4>회의 내용</h4><p><MeetingText>{meeting.discussion}</MeetingText></p></section>{meeting.decisions && <section><h4>결정사항</h4><p><MeetingText>{meeting.decisions}</MeetingText></p></section>}{meeting.actionItems && <section className="is-action"><h4>후속 업무</h4><p><MeetingText>{meeting.actionItems}</MeetingText></p></section>}</div></article>)}</section> : <EmptyState title="작성된 회의록이 없습니다" description={canWrite ? "오늘 회의 내용을 첫 기록으로 남겨 주세요." : "운영팀이 회의록을 작성하면 이곳에 표시됩니다."} />}
...
export function App() {
  ...
  const copyTasks = async (selectedTasks) => {
    ...
    const projectId = activeProjectId;
    const key = JSON.stringify([projectId, selectedTasks.map(task => task.id)]);
    let plan = copyPlansRef.current.get(key);
    if (!plan) {
      const current = resourceCacheRef.current.get(`${projectId}:tasks`)?.state?.data?.items || resourceState.data?.items || [];
      plan = createTaskCopyPlan(selectedTasks, Math.max(0, ...current.map(task => Number(task.sortOrder) || 0)));
      copyPlansRef.current.set(key, plan);
    }
    ...
    try {
      ...
      copyPlansRef.current.delete(key);
      setSaveNotice(`${selectedTasks.length}개 업무를 복사했습니다.`);
    } finally {
  ...
  const openNotificationTask = (task) => {
    if (task?.entityType === "PROJECT_ISSUE" && role !== "client") { setNotificationIssue(task); return; }
    if(task?.projectId) openDashboardProject(task.projectId, "schedule");
    else navigateToView("schedule");
