...
export default function WorkspaceNotifications({source,actorId,onSelect,issues=[]}) {
  ...
  const load=async(cursor=null)=>{
    ...
    try {
      ...
      const items=(result.data.items||[]).filter(item=>isNewTask({createdAt:item.created_at})).map(item=>({...item,actorName:result.data.actors?.find(actor=>actor.id===item.actor_user_id)?.display_name||"작성자 미확인"}));
  ...
  const items=state.items.filter(item=>isNewTask({createdAt:item.created_at})).map(item=>({...item,issue: item.entity_type==="PROJECT_ISSUE" ? issues.find(issue=>String(issue.id)===String(item.entity_id)&&String(issue.projectId)===String(item.project_id)) : null}));
  const unread=items.filter(item=>!read.includes(String(item.id)));
  return <div className="notification-center" ref={root}><button type="button" className="notification-trigger" aria-label={'전체 프로젝트 알림, 미확인 '+unread.length+'건'+(state.cursor?' 이상':'')} aria-expanded={open} aria-haspopup="dialog" onClick={()=>setOpen(value=>!value)}><Bell size={17}/>{unread.length>0&&<span className="notification-count">{unread.length}{state.cursor?'+':''}</span>}</button>{open&&<section className="notification-popover" role="dialog" aria-label="전체 프로젝트 알림"><header><div><strong>전체 프로젝트 알림</strong><span>최근 24시간 새 업무·확인요청 · 조회 권한 내 전체 프로젝트</span></div>{unread.length>0&&<button onClick={()=>acknowledge(items.map(item=>item.id))}>표시된 알림 확인</button>}</header><div className="notification-list">{state.error?<div className="notification-empty" role="alert"><span>{state.error}</span><button onClick={()=>void load()}>다시 시도</button></div>:items.map(item=><button className={'notification-item'+(!read.includes(String(item.id))?' is-unread':'')} key={item.id} onClick={()=>{acknowledge([item.id]);setOpen(false);onSelect({id:item.entity_id,projectId:String(item.project_id),entityType:item.entity_type,projectName:item.project?.project_name||'',clientName:item.issue?.clientName||'',title:item.task_detail?.task_title||''});}}><span className="notification-item-mark"/><span><small>{item.project?.project_name||'프로젝트'}</small><strong>{item.entity_type==='PROJECT_ISSUE'?'확인요청 · '+(item.issue?.relatedTask||item.issue?.body||'#'+item.entity_id):item.task_detail?.task_title||'새 업무 #'+item.entity_id}</strong>{item.issue?.body&&<small>{item.issue.body}</small>}<small>{item.actorName} · {new Intl.DateTimeFormat('ko-KR',{timeZone:'Asia/Seoul',month:'numeric',day:'numeric',hour:'2-digit',minute:'2-digit'}).format(new Date(item.created_at))} 등록</small></span></button>)}{state.loading&&<div className="notification-empty" role="status">알림을 불러오는 중입니다.</div>}{!state.loading&&!state.error&&!items.length&&<div className="notification-empty">최근 24시간 새 업무·확인요청 알림이 없습니다.</div>}{state.cursor&&<button disabled={state.loading} onClick={()=>void load(state.cursor)}>알림 더 보기</button>}</div><footer>확인 상태는 이 계정의 현재 브라우저 탭에만 저장됩니다.</footer></section>}</div>;
}
