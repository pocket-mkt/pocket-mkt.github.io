...
function IssueDetailModal({ issue, canWrite, actorName, onUpdate, onUpdated, onClose }) {
...
export function OperationsDashboardView({ dashboard, canWrite, actorName, onIssueUpdate, onOpenProject, onLoadWeek }) {
  ...
  const effectiveIssues = useMemo(() => dashboard.issues.map((item) => issueOverrides[item.id] || item), [dashboard.issues, issueOverrides]);
  const filteredIssues = effectiveIssues.filter((item) => !["DONE", "CLOSED", "COMPLETED"].includes(item.statusCode) && (projectFilter === "all" || item.projectId === projectFilter));
