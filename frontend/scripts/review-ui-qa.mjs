...
window.runQa = async () => {
 ...
 document.querySelector('.ops-completion-load').click();await tick();
 ...
 await render(<DeadlineTaskLink task={{id:'2',completionUrl:''}}/>);check(document.querySelector('.ops-completion-empty').textContent==='미등록','empty link state missing');
 ...
 results.push('deadline completion links: deferred lookup, safe external link, missing/unsafe distinctions');
