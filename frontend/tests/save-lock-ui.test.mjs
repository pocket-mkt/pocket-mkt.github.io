...
test("업무·이슈·프로젝트·회의록·계정대장·KPI·권한 저장이 공통 잠금을 우회하지 않는다", () => {
  assert.equal((appSource.match(/await mutateWithSaveLock\(/g) || []).length, 12);
  assert.equal((appSource.match(/await mutateBatchWithSaveLock\(/g) || []).length, 2);
